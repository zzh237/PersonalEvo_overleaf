...
\end{equation}
because each candidate's reward parameter is estimated by an independent isotropic ridge regression. When the routing gain is large this cost is easily absorbed; but the base-method measurements show that in realistic archives the dominance margin \eqref{eq:intro-dominance} can be as thin as $+0.022$ (GEPA prompt archives) or $+0.027$ (BESPOKE). In such regimes exploration nearly erases the value of personalization, and any reduction in $\bar L_T$ directly widens the set of applications where routing pays off.

The isotropic prior in \eqref{eq:intro-linucb-cost} treats archive candidates as unrelated arms. Candidates in a self-evolving archive are not unrelated: prompt variants, skill modules, and evolved descendants share latent structure, since children are mutations of parents and candidates within a category act on the same feature directions. Our thesis is a single sentence: \emph{self-evolving agents should meta-learn the geometry of their archive}. Concretely, we model each candidate's reward parameter as a random effect drawn from a shared hyper-covariance,
\begin{equation}
...
\section{Base Method: PersonalEvo Per-Candidate LinUCB Router (Background)}
...
The remainder of this section formalizes the notation, presents the algorithm, and states the main theoretical results.
