...
\begin{abstract}
Self-evolving language-model agents maintain archives of prompts, skills, or
policies. Deploying one candidate for every user ignores preference
heterogeneity, whereas learning to route users online incurs exploration
whenever the archive grows. We introduce HyperRidge-Base (HRB), a contextual-bandit
router that personalizes candidate selection and can add candidates after
failures. Its average reward decomposes exactly into the reward of the best
fixed initial candidate, a routing gain, an evolution gain, and an exploration
cost. Personalization therefore improves over fixed deployment exactly when
the two gains exceed the learning cost.

We then introduce HyperRidge, which regularizes
candidate-specific reward models with a shared archive covariance. For known
covariance, we prove a pathwise exploration bound governed by an
effective-dimension potential and an explicit confidence radius. We also prove
finite-sample control for a calibration-split estimator that learns the
covariance from fresh candidates and freezes it before routing. These
guarantees do not cover periodic online re-estimation.

Across nine synthetic, agent-framework, and real-preference benchmarks,
routing gain ranges from $0.036$ to $0.280$ and exceeds evolution gain in every
case. HRB outperforms fixed and random deployment on all nine. The
base experiments report an observed-reward residual rather than the theorem's
mean-reward exploration cost. In separate planted rank-three diagnostics,
known geometry reduces cumulative exploration cost by $46$--$59\%$, while
periodic re-estimation increases it by $7$--$19\%$. These results identify both
the value of personalized routing and the remaining challenge: estimating
useful shared geometry from deployable candidate features.
\end{abstract}
...
We address this question in two steps. First, \emph{HyperRidge-Base} (HRB) separates the value of personalization from the cost of learning it. Let $A_1$ be the initial archive and let $R_T$ be the router's average reward over $T$ interactions. We prove the exact decomposition
\begin{equation}
...
Second, \emph{HyperRidge} targets the only negative term in \eqref{eq:intro-decomp}. Each candidate $a$ has its own parameter vector $\theta_a$, which maps $d$ user and candidate features to expected reward. A disjoint LinUCB router estimates each vector with the same spherical regularizer and therefore encodes no relation among candidates \citep{abbasi2011oful,chu2011linucb}. We instead model the candidate vectors as draws with a shared covariance $\Omega$. This covariance is the archive geometry. Its dominant directions capture recurring factors such as tone and topic, while every candidate keeps its own reward model. HyperRidge uses this geometry to regularize candidate estimates, so a newly evolved candidate starts with uncertainty shaped by the archive rather than an isotropic blank slate.
...
Our experiments separate the motivation for HyperRidge from evidence for its mechanism. Across nine benchmarks, HRB has routing gains from $0.036$ to $0.280$, evolution gains from $0$ to $0.061$, and observed-reward residuals from $0.010$ to $0.071$. Because the simulator clips noisy rewards, these residuals are not identified with the theorem's mean-reward exploration cost. On separate planted rank-three problems, giving HyperRidge the true geometry lowers cumulative exploration cost by $46$--$59\%$. Periodically estimating that geometry instead raises the cost by $7$--$19\%$. On seven adapter benchmarks, one-hot candidate features make the full-covariance router identical to its diagonal ablation, so their reward changes cannot show cross-candidate transfer. The evidence therefore supports the known-geometry mechanism while exposing the unresolved estimation and representation problem.
...
\item \textbf{Archive-aware routing.} We introduce HyperRidge, which learns a covariance geometry across candidate reward models and uses it in a generalized-ridge UCB router. It shares evidence about important feature directions while retaining candidate-specific predictions.
...
\item \textbf{Controlled empirical evaluation.} We measure the structural terms and report the observed-reward residual on nine benchmarks, then test HyperRidge in a controlled low-rank setting. The results show when known geometry helps and why periodic estimation with one-hot features does not recover that gain.
\end{enumerate}

\paragraph{Paper organization.}
Section~\ref{sec:related} positions the work relative to personalization, agent evolution, routing, and linear bandits. Section~\ref{sec:background} introduces the routing problem and notation; Section~\ref{sec:method} presents the base and HyperRidge routers; and Section~\ref{sec:theory} develops the decomposition and effective-dimension analysis. Section~\ref{sec:experiments} then reports the base-router results, HyperRidge diagnostics, and remaining evaluation gap. Appendix~\ref{app:hyper-proofs} proves the oracle geometry results, and Appendix~\ref{app:err-control-proof} gives the calibration-split covariance theorem.
...
These route at the \emph{model} level over a fixed pool; HyperRidge routes \emph{per user} over an archive that may grow online, and decomposes the resulting reward rather than only reporting it.
...
\section{Preliminaries: Personalized Archive Routing}
\label{sec:background}

\subsection{Overview}

\label{sec:method-plain}
Managed platforms such as Microsoft Foundry Agent Service and Amazon Bedrock AgentCore \citep{microsoft2026foundryagents,aws2026agentcore} support agents with different models, tools, and instructions. They motivate the deployment setting; we do not assume that either platform implements our router.
...
A user $U \sim \Pi$ is drawn from the user space $\mathcal{U}$. This space is a finite set of profiles in our benchmarks and may be a general measurable space in the analysis. The distribution $\Pi$ specifies the user population, and $\E_U[\cdot]$ averages over it.
...
HyperRidge changes how the router estimates candidate-specific reward models while leaving the archive accounting above unchanged.
...
The response vector $y_{a,t}$ contains centered observations $r_s-b(u_s)$, not unknown mean rewards. Row $s$ of $X_{a,t}$ contains the features of the user served in that round, so a shared router may pool observations across users. Algorithm~\ref{alg:hyper-ridge} allows such a user stream. The four-term decomposition instead follows one user and then averages over the population; combining it with the HyperRidge bound requires $u_t\equiv U$. The confidence and potential lemmas hold for a predictable user stream, but the four-term conclusion does not automatically extend to it.
...
The per-round cost is identical to HRB up to the
...
Section~\ref{sec:experiments} reports the nine-benchmark results for
HRB and separates two HyperRidge questions: whether a known
archive geometry can reduce exploration, and whether the current periodic
estimator can recover that geometry from online data.

\subsection{Base routing and archive growth}
...
The average reward of Algorithm~\ref{alg:router} over horizon $T$ satisfies
...
\(m_t(u)\) and the selected mean \(\mu(u,a_t(u))\). The reported tables use
...
\subsection{Exploration-cost bounds}

\label{sec:method-assump}

We use the setup of Section~\ref{sec:background} and Assumptions~\ref{ass:bounded}--\ref{ass:bounded-feat} below, writing $x(u,a)$ for the feature map $\psi(u,a)$. We first establish the decomposition and base-router bound, then introduce the shared geometry in Assumption~\ref{assump:shared-structure}.
...
\(m_t(u)-\mu(u,a_t(u))\). The headline results instead report
...
What is specific to HyperRidge is the four-term accounting for a user drawn
...
We now turn to the new result. HyperRidge leaves the four-term decomposition
...
confidence radius and an elliptical-potential term. HyperRidge can reduce the
...
The routing rule analyzed below uses the generalized design
\eqref{eq:hyper-design}, estimate \eqref{eq:hyper-theta-hat}, and score
\eqref{eq:hyper-ucb} defined in Section~\ref{sec:method}; Algorithm~\ref{alg:hyper-ridge}
...
The simulator knows every pre-clipping mean, so the theoretical exploration cost could be computed from the selected candidate's mean. The headline tables instead use the observed residual \(m_t-r_t\). We keep that quantity separate below; in a real system \(m_t(u)\) would also require held-out estimation.
...
\end{tabular}
\endgroup
\captionof{table}{Benchmark characteristics. $|\mathcal{U}|$: users; $|A_1|$: candidates. Synthetic: $T{=}300$; others: $T{=}250$.}
\label{tab:envs}
\end{minipage}
\hfill
...
We compare HRB and four component ablations of Algorithm~\ref{alg:router}: \textbf{HRB} (ordinary ridge, routing, and gated reflection), \textbf{No-routing} (uniform-random candidate selection), \textbf{No-reflection} (frozen archive), \textbf{No-personalization} (onehot-only features), and \textbf{Collapse-1} (single candidate).

The main figures show HRB, No-routing, and No-reflection against the Static
...
\paragraph{Ablation.} Figure~\ref{fig:ablation} compares HRB, No-routing, and
...
The plotted means satisfy No-routing $<$ Static $<$ No-reflect $<$ HRB $<$ Oracle on every benchmark.

\begin{figure}[h]
\centering
% Replace only the legacy method label; preserve the registered plot raster.
...
No-reflect, HRB, and Oracle from left to right. No-routing and No-reflect
...
In all cases, HRB converges toward the oracle within ${\sim}50$ rounds, while the static baseline remains flat.
...
To position HRB against prior routing strategies, we compare five methods on all nine benchmarks (Table~\ref{tab:baselines}): (i)~\textbf{Static}: deploy the single best-on-average candidate; (ii)~\textbf{GEPA-Pareto}: maintain the Pareto-optimal subset of the archive and deploy uniformly from it \citep{agrawal2025geparp}; (iii)~\textbf{Thompson}: Bayesian Thompson sampling over candidates \citep{pahf2025}; (iv)~\textbf{RouteLLM-thresh}: route users to specialist or generalist candidates based on a variance threshold; (v)~\textbf{HRB}: the ordinary-ridge base router with gated reflection.
...
\toprule
\textbf{Benchmark} & \textbf{Static} & \textbf{GEPA-Pareto} & \textbf{Thompson} & \textbf{RouteLLM} & \textbf{HRB} & \textbf{Oracle} \\
\midrule
...
HRB combines per-user routing with gated evolution and a small logged observed-reward residual. The experiment does not identify that residual with the theoretical \(\bar L_T\).

\subsection{HyperRidge diagnostics and the remaining evaluation gap}
...
Table~\ref{tab:baselines} all evaluate HRB, not HRP. We
additionally ran a five-seed diagnostic suite for HyperRidge. We
...
With tuning seeds disjoint from the five reporting seeds, the periodic estimator improves mean reward over the isotropic router on six of seven adapters, with changes ranging from $+0.002$ on BESPOKE to $+0.048$ on Hermes, and loses $0.010$ on GEPA. These numbers do not establish cross-candidate geometry transfer. The diagnostic represents candidate $a$ by the one-hot vector $e_a$. Starting from $\widehat{\Omega}_1=I$, each candidate estimate has support only on its own coordinate, so the covariance update remains diagonal. The full-covariance and diagonal-ridge variants therefore match on every adapter and seed. More seeds cannot resolve this restriction of the feature map.

\paragraph{Scope of comparison.}
Assessing covariance estimation requires comparing base, known-covariance, calibration-split
...
directions. The comparison must report all four mean-reward terms of
...
\(d_{\mathrm{eff}}(\widehat\Omega;X)\). The diagnostics above do not establish
an empirical advantage for the fully adaptive HyperRidge router.

\section{Conclusion}

\label{sec:conclusion}

HyperRidge studies the tradeoff between personalized routing, archive growth,
and exploration. Theorem~\ref{thm:decomp} and Corollary~\ref{cor:dominance}
express this tradeoff relative to fixed deployment.
HyperRidge preserves the form of the decomposition and targets the
exploration cost. It models candidate parameters with a shared geometry
...
\(O(\sqrt{T}\log T)\). Geometry helps only when the complete HyperRidge budget
...
Two limitations bound the empirical claims. First, the nine-benchmark estimates, ablation hierarchy, per-round traces, and baseline comparison in Section~\ref{sec:experiments} belong to the \emph{base} LinUCB router. Their logged margins are
\begin{equation}
...
logged to evaluate \(\bar L_T\) itself. The separate HyperRidge suite is
...
The comparison in Section~\ref{sec:exp-hyper-ridge-plan} requires base LinUCB, known-covariance, calibration-split, periodically estimated, and diagonal-ridge routers evaluated with shared features and a common protocol. Its primary endpoint is the mean-reward exploration cost $\widehat L_T^\mu$, with $\widehat L_T^{\mathrm{obs}}$ reported separately. Covariance error, the fitted regret exponent, and $d_{\mathrm{eff}}(\widehat\Omega;X)$ distinguish regimes where
...
budget determines the comparison. The experiments reported here do not
evaluate the calibration-split procedure under its diversity assumptions.

The proposer in the simulated environments uses oracle information, so the measured evolution gain $\bar G_T$ does not estimate the performance of an LLM-driven proposer. Each user also maintains a private archive; candidates discovered for one user are not shared with others. Sharing an archive could provide more data for estimating \(\Omega\), but would require revisiting the predictability argument in Lemma~\ref{lem:predictable}. The decomposition \eqref{eq:thm-decomp-avg} applies more broadly to model routing, prompt selection, and tool-use policies that choose from a candidate library.
...
\Delta_{\mathrm{route}}(e) = \bar{\mathcal{V}}_T(e,\mathrm{HRB}) - \bar{\mathcal{V}}_T(e,\mathrm{NR}).
\end{equation}
...
\Delta_{\mathrm{refl}}(e) = \bar{\mathcal{V}}_T(e,\mathrm{HRB}) - \bar{\mathcal{V}}_T(e,\mathrm{NF}),
\end{equation}
...
\Delta_{\mathrm{pers}}(e) = \bar{\mathcal{V}}_T(e,\mathrm{HRB}) - \bar{\mathcal{V}}_T(e,\mathrm{NP}),
\end{equation}
...
The HRB per-cluster numbers (averaged across $5$ seeds, with the standard deviation across seeds) are listed in Table~\ref{tab:app-per-cluster}.
...
\subsection{Sensitivity to noise scale $\sigma$}
...
\subsection{Sensitivity to initial archive size $K$}
...
Algorithm~\ref{alg:disjoint-linucb} gives the complete per-user procedure,
including parameter initialization when reflection appends a candidate.
...
The five ablations in Section~\ref{sec:experiments} are defined as follows:
...
The no-personalization variant drops $w_u$ from $\psi$ but still gives each user a private bandit and a private archive.
...
The complete serialized trajectory data for the headline grid is approximately \(6\) MB.
...
The \texttt{collapse\_1} variant uses $K=1$ and requires no matrix inversion.
The \texttt{no\_routing} variant replaces the $\arg\max$ with a uniform draw
...
\section{Detailed Synthetic Results}

\label{app:tables}

This appendix reports per-seed, per-cluster, and per-user results for the synthetic environments.
...
\begin{aligned}
\mathcal{V} \;=\; \{\,\textnormal{HRB},\; \texttt{no\_routing},\; \texttt{no\_reflection},\; \texttt{no\_personalization},\; \texttt{collapse\_1}\,\},
\end{aligned}
\end{equation}
...
\subsection{Full decomposition table}
...
This appendix block analyzes the new HyperRidge router.  The reward accounting is inherited unchanged from Theorem~\ref{thm:decomp} and Corollary~\ref{cor:dominance}; we do not re-prove the four-term decomposition.  The only question addressed here is how replacing ordinary ridge geometry by a shared archive covariance changes the exploration-cost term.
...
and~\ref{ass:hyper-oracle-calibrated}, let the oracle HyperRidge router use
...
\midrule
Exact HyperRidge accounting & \(V_{\mathrm{static}}+\Delta_{\mathrm{route}}+\bar G_T-\bar L_T\) & inherited from Theorem~\ref{thm:decomp} \\
...
Empirical validation of HyperRidge gains & HRK, HRC, HRP, HRD, and HRB & diagnostic only; full test remains open \\
\bottomrule
\end{tabular}
\caption{Theory map for HyperRidge. The oracle and
