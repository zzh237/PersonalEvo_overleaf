...
\begin{abstract}
Language-model agents can personalize their behavior by choosing among
prompts, skills, or policies. Learning a separate reward model for every
candidate incurs exploration, especially when new candidates enter the
archive. We introduce HyperRidge, a contextual-bandit router that retains
candidate-specific predictors but regularizes them with a shared covariance.
The covariance describes how candidate parameters vary across feature
directions and shapes both estimation and uncertainty. For known geometry,
we prove a cumulative exploration-cost bound with an effective-dimension
potential and an explicit confidence radius. For an independently calibrated
and frozen covariance, we prove finite-sample error control and a routing
bound. Neither guarantee covers periodic online re-estimation.

We evaluate known geometry, matched covariance controls, and the practical
periodic method. On planted rank-three problems, known geometry reduces
exploration cost by $46$--$59\%$, but periodic estimation increases it.
Across nine synthetic and proxy environments, matched controls show lower
exploration cost than identity regularization without a consistent advantage
over scalar or diagonal covariance. A separate $600$-case real-LLM study
finds $67.47\%$ preference accuracy for the periodic method, compared with
$77.47\%$ for ordinary ridge. These results distinguish the value of correct
geometry from the unresolved difficulty of learning it under adaptive
candidate selection.
\end{abstract}
...
Profile and memory methods condition agents on users
\citep{salemi2023lamp,jin2026personalizing,zhang2025personaagent,westhausser2025persistent};
evolution methods search for stronger candidates
\citep{fernando2023promptbreeder,guo2024evoprompt,zhou2024symbolic,agrawal2025geparp};
and fixed-pool routers learn assignments
\citep{ong2024routellm,li2025llmbandit,tsai2026neuralucb}.
...
We first analyze supplied geometry, then independently estimated geometry.
HyperRidge-Known (HRK) has a finite-time exploration bound governed by a
geometric potential and its full confidence radius.
HyperRidge-Calibrated (HRC) estimates covariance from fresh calibration
candidates and freezes it before routing. Its additional calibration and
width-inflation charge is $o(\sqrt T)$ under the stated schedule; its leading
confidence radius is not the oracle radius.
HyperRidge-Periodic (HRP) is the practical update that re-estimates geometry
from online candidate models. The HRK and HRC guarantees do not apply to
that adaptive update.
...
consistently outperform simpler adaptive regularizers, and HRP underperforms
...
for HRK and HRC under explicit observation, diversity, and confidence
...
Sections~\ref{sec:background}--\ref{sec:theory} present the routing problem,
HyperRidge, and its known-geometry and calibration guarantees.
Section~\ref{sec:experiments} evaluates geometry, the complete method, and
base components; Section~\ref{sec:related} discusses related work.
Appendices~\ref{app:hyper-proofs}--\ref{app:err-control-proof} give the
geometry proofs. Reward accounting and detailed component results follow.
...
assumption used to prove it. The identity, examples, and full conditional
comparison appear in Appendix~\ref{app:reward-support}.
...
validate Theorem~\ref{thm:err-control-calibration-split-final}.
...
by the HRK or HRC guarantees, and the current holdout experiment does not
satisfy the HRC theorem's calibration assumptions. The fixed-user feature
...
HRC empirical validation & independent full-rank calibration and frozen archive & not evaluated under the theorem's assumptions \\
\bottomrule
\end{tabular}
