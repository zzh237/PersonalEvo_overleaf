...
whenever the archive grows. We introduce PersonalEvo, a contextual-bandit
...
We then introduce Hyper-Ridge PersonalEvo, which regularizes
...
case. PersonalEvo outperforms fixed and random deployment on all nine. The
saved base runs report an observed-reward residual rather than the theorem's
...
Section~\ref{sec:related} positions the work relative to personalization, agent evolution, routing, and linear bandits. Section~\ref{sec:background} introduces the routing problem and notation; Section~\ref{sec:method} presents the base and Hyper-Ridge routers; and Section~\ref{sec:theory} develops the decomposition and effective-dimension analysis. Section~\ref{sec:experiments} then reports the base-router results, Hyper-Ridge diagnostics, and remaining evaluation gap. Appendix~\ref{app:hyper-proofs} proves the oracle geometry results, and Appendix~\ref{app:err-control-proof} gives the calibration-split covariance theorem.
...
\section{Preliminaries: PersonalEvo Routing}
...
The remainder of this section formalizes the routing notation and the accounting quantities used by the method and theory.
...
Hyper-Ridge changes how the router estimates candidate-specific reward models while leaving the archive accounting above unchanged.
...
\end{equation}
\subsection{Hyper-Ridge routing}
...
Section~\ref{sec:experiments} reports the nine-benchmark results for the base
PersonalEvo router and separates two Hyper-Ridge questions: whether a known
...
\subsection{Proof of the Hyper-Ridge dominance corollary}
