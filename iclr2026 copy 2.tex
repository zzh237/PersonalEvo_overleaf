...
We evaluate PersonalEvo on nine benchmarks: three synthetic personalized-agent environments, two agent-framework substrates (GEPA prompt archives \citep{agrawal2025geparp}, Hermes skill configurations \citep{hermes_selfevolution}), and four real-data preference datasets (BESPOKE \citep{jin2026personalizing}, PAHF Shopping and Embodied \citep{pahf2025}, and PrefEval \citep{zhao2025prefeval}).

Across all nine, PersonalEvo achieves higher mean reward than both the static baseline and random candidate selection.

Our results show that: (1)~the routing gain $\Delta_{\mathrm{route}}$ is the dominant source of improvement, contributing 85--95\% of the reward difference over uniform deployment; (2)~on the four real-data benchmarks, $\Delta_{\mathrm{route}}$ ranges from $0.04$ to $0.26$, indicating that the value of personalized routing varies substantially across application domains; (3)~growing the archive without gating hurts performance, because the added exploration cost outweighs the evolution gain.

Building on this base, we introduce \emph{Hyper-Ridge PersonalEvo}, which replaces the router's isotropic ridge prior with a meta-learned archive geometry $\Omega$ that captures shared latent structure across candidates. Under a low-dimensional shared-structure assumption, we state a proposition that the exploration cost scales with an effective dimension $d_{\mathrm{eff}}(\Omega)$ rather than the ambient dimension $d$; a full proof controlling the covariance-estimation error term $\mathrm{Err}_T(\widehat{\Omega},\Omega)$ remains open. The new router is implementation-ready in our codebase, and we lay out a concrete experimental plan, with exact metrics and artifact names, to test whether the meta-learned geometry reduces the measured exploration cost relative to the base method.
\end{abstract}
...
A personalized agent maintains a small library of candidate prompts, skills, or orchestration policies, called an \emph{archive}, and must decide what to serve to each incoming user. The simplest strategy is uniform deployment: serve everyone the single candidate with the best population-average reward. The alternative is personalized routing: learn, online, which candidate suits which user. Routing exploits user heterogeneity but pays an exploration cost while learning. The base PersonalEvo framework, which this paper builds on and extends, resolves this trade-off with an exact accounting identity. Writing $A_1$ for the common initial archive and $R_T$ for the router's average reward over a horizon of $T$ rounds, the identity is
\begin{equation}
...
\bar L_T .
\end{equation}
Here $V_{\mathrm{static}}(A_1)$ is the reward of the best single candidate served to everyone, $\Delta_{\mathrm{route}}(A_1)\ge 0$ is the \emph{routing gain} from matching each user to their own best candidate, $\bar G_T$ is the \emph{evolution gain} from candidates discovered online, and $\bar L_T$ is the \emph{exploration cost} paid while learning user preferences. The identity yields a falsifiable dominance condition: personalized routing beats uniform deployment exactly when
\begin{equation}
\label{eq:intro-dominance}
\Delta_{\mathrm{route}}(A_1) + \bar G_T \,>\, \bar L_T .
\end{equation}
The base method instantiates the router as disjoint LinUCB over the archive and validates \eqref{eq:intro-decomp} on nine benchmarks (three synthetic environments, two agent-framework substrates, four real-data preference datasets). All empirical results in this paper are for that base LinUCB router: it achieves mean reward $0.68$--$0.80$ against $0.53$--$0.65$ for the best fixed candidate, with measured routing gain $\Delta_{\mathrm{route}} \in [0.04, 0.26]$ and exploration cost $\bar L_T \in [0.03, 0.08]$, and the routing gain accounts for $85$--$95\%$ of the improvement in every benchmark.

The decomposition \eqref{eq:intro-decomp} also exposes a precise weakness. Three of its four terms are non-negative; the only term working against personalization is $\bar L_T$. For disjoint LinUCB with archive cap $K_{\max}$ and feature dimension $d$, the standard self-normalized analysis \citep{abbasi2011oful,chu2011linucb} controls the cumulative exploration cost at the rate
\begin{equation}
\label{eq:intro-linucb-cost}
\sum_{t=1}^{T}\E[L_t(U)]
\,=\,
O\!\left(\sqrt{K_{\max}\, d\, T \log T}\right),
\end{equation}
because each candidate's reward parameter is estimated by an independent isotropic ridge regression \zz{have you bring the model, and have you mentioned the paramter of K, d, T?}. When the routing gain is large this cost is easily absorbed; but the base-method measurements show that in realistic archives the dominance margin \eqref{eq:intro-dominance} can be as thin as $+0.022$ (GEPA prompt archives) or $+0.027$ (BESPOKE). In such regimes exploration nearly erases the value of personalization, and any reduction in $\bar L_T$ directly widens the set of applications where routing pays off.


The isotropic prior in \eqref{eq:intro-linucb-cost} treats archive candidates as unrelated arms \zz{it seems the euqation 3 does not have an isotropic prior}. Candidates in a self-evolving archive are not unrelated: prompt variants, skill modules, and evolved descendants share latent structure, since children are mutations of parents and candidates within a category act on the same feature directions. Our thesis\zz{what do you mean here?} is a single sentence: \emph{self-evolving agents should meta-learn the geometry of their archive}. Concretely, we model each candidate's reward parameter as a random effect drawn from a shared hyper-covariance, \zz{here, the notations have never been introduced, so interms of writing, if it is in the intro part, we need to introduce these terms one by one, say, we let theta a be ... represent ... and, so what is the X, what is a, what is the t, what is the y, what is the lambda, what is the theta a , what is the omega t, waht is the d, what is the d eff, these need to be formally introduced in the notation section, but here, it also need to have a easy human real world case application meaning}
\begin{equation}
\label{eq:intro-random-effect}
\theta_a \,\sim\, (0,\, \Omega),
\qquad
\Omega \in \mathbb{R}^{d\times d},
\end{equation}
and replace each per-candidate isotropic ridge estimate with a generalized ridge estimate whose regularizer is the estimated archive geometry $\widehat\Omega_t$:
\begin{equation}
\label{eq:intro-hyper-ridge}
\widehat\theta_{a,t}
\,=\,
\argmin_{\theta \in \mathbb{R}^d}
\left\{
\|y_{a,t} - X_{a,t}\theta\|_2^2
+
\lambda\, \theta^\top \widehat\Omega_t^{-1}\theta
\right\}.
\end{equation}
The resulting router, \emph{Hyper-Ridge PersonalEvo} (Algorithm~\ref{alg:hyper-ridge}), keeps the decomposition \eqref{eq:intro-decomp} literally unchanged, because \eqref{eq:intro-decomp} is an identity that holds for any measurable routing rule. What changes is the theory of the exploration term. Under a low-dimensional shared-structure \zz{what do you mean shared-structure? } condition (Assumption~\ref{assump:shared-structure}), the ambient dimension $d$ in \eqref{eq:intro-linucb-cost} is replaced by the effective dimension induced by the archive geometry,
\begin{equation}
\label{eq:intro-deff}
d_{\mathrm{eff}}(\Omega)
\,=\,
...
X^\top X
\left(X^\top X + \lambda\,\Omega^{-1}\right)^{-1}
\right],
\end{equation}
yielding the anticipated exploration-cost bound
\begin{equation}
...
\end{equation}
where $\mathrm{Err}_T(\widehat\Omega,\Omega)$ collects the price of estimating the geometry from archive data. As a rough calibration: with the base paper's feature dimension $d = 10$, an archive whose candidates concentrate on a two-dimensional shared subspace has $d_{\mathrm{eff}}(\Omega) \approx 2$, so the leading term of \eqref{eq:intro-hyper-cost} shrinks by a factor of roughly $\sqrt{5} \approx 2.2$ relative to \eqref{eq:intro-linucb-cost}. On a thin-margin benchmark where $\bar L_T \approx 0.025$ and the dominance margin is $+0.022$, halving the exploration cost roughly doubles the margin. Archive evolution thus becomes doubly useful: it supplies better candidates ($\bar G_T$) and it supplies more data for estimating $\widehat\Omega_t$, which sharpens the router's regularization geometry and shrinks $\bar L_T$.

We are explicit about the epistemic status of each claim, since the two halves of this paper rest on different evidence. The four-term decomposition, the dominance condition, and all nine-benchmark measurements are inherited base-method results, proven and reported for the LinUCB router. The effective-dimension improvement is stated as Proposition~\ref{prop:deff-bound}: it is a conditional result whose oracle-geometry part follows from the standard elliptical-potential argument with the generalized design matrix, while a complete finite-sample control of the covariance-estimation term $\mathrm{Err}_T(\widehat\Omega,\Omega)$ in \eqref{eq:intro-hyper-cost} remains open; the proof status is detailed in Appendix~\ref{app:hyper-proofs}. Likewise, no hyper-ridge experiments have been run: the current codebase implements the base router, the hyper-ridge extension is implementation-ready, and Section~\ref{sec:experiments} specifies a complete pre-registered comparison grid (base LinUCB, oracle-covariance hyper-ridge, estimated-covariance hyper-ridge, and a diagonal-ridge ablation, plus heterogeneity and low-rank geometry sweeps) with exact metrics and artifact names.

\subsection{Motivation}

The original PersonalEvo decomposition writes the average reward as

\[
R_T = \Rstatic(A_1) + \Droute(A_1) + \bar{G}_T - \bar{L}_T .
\]

Here $\bar{L}_T$ is the exploration cost.

Thus, a natural way to improve
PersonalEvo is not necessarily to change the decomposition, but to reduce the
negative term $\bar{L}_T$.

The current PersonalEvo router treats archive candidates as independent LinUCB
arms.

However, candidates in a self-evolving archive are not independent:
prompt variants, skill modules, and evolved descendants often share latent
structure.

We therefore propose to meta-learn the geometry of the archive.

\begin{center}
\fbox{\textbf{Self-evolving agents should meta-learn the geometry of their archive.}}
\end{center}

\subsection{Why this is new}

The original PersonalEvo method routes users over an archive.

Hyper-Ridge
PersonalEvo additionally learns the geometry of the archive itself.

Archive
growth is no longer only a source of better candidates; it also provides data
for estimating $\widehat{\Omega}_t$, which improves the router's
regularization geometry.

Thus archive evolution creates two benefits:

\[
\text{better candidates} \quad + \quad \text{better covariance geometry}.
\]
...
The method changes the question from

\begin{center}
\emph{Which candidate should I try for this user?}
\end{center}

to

\begin{center}
\emph{What is the latent geometry of all candidates, and how should it guide
exploration?}
\end{center}

This is important because self-evolving archives are structured objects, not
unordered bags of prompts.

Hyper-covariance estimation makes this structure
explicit.

\subsection{Contribution statement}

A possible contribution statement is:

\begin{quote}
We extend PersonalEvo by introducing Hyper-Ridge PersonalEvo, a meta-learning
router that estimates a hyper-covariance over archive candidates and uses it
as the generalized ridge geometry in LinUCB. This preserves the exact
PersonalEvo reward decomposition while sharpening the exploration-cost term
from a dimension-dependent bound to an archive-geometry-dependent
effective-dimension bound. As a result, archive evolution improves both
candidate quality and statistical efficiency.
\end{quote}


\paragraph{Contributions.}
This paper makes four contributions. First, it introduces the Hyper-Ridge PersonalEvo router (Algorithm~\ref{alg:hyper-ridge}), which meta-learns a hyper-covariance over archive candidates and uses it as the generalized-ridge geometry inside the UCB routing rule, while preserving the exact decomposition \eqref{eq:intro-decomp} and the base framework's notation and assumptions. Second, it develops the effective-dimension theory of the exploration cost: under Assumption~\ref{assump:shared-structure}, Proposition~\ref{prop:deff-bound} replaces the $\sqrt{d}$ dependence of \eqref{eq:intro-linucb-cost} with $\sqrt{d_{\mathrm{eff}}(\Omega)}$ up to the estimation term $\mathrm{Err}_T(\widehat\Omega,\Omega)$, with the remaining proof gap stated precisely. Third, it consolidates the base-method evidence, attributing the nine-benchmark decomposition measurements, ablations, and baseline comparisons to the LinUCB router and reading them through the lens of \eqref{eq:intro-hyper-cost}: the thin-margin benchmarks identify exactly the regimes where a geometry-aware router should matter most. Fourth, it provides an implementation-ready experimental protocol for validating the effective-dimension prediction, including the $d_{\mathrm{eff}}$ diagnostic that ties each planned run back to Proposition~\ref{prop:deff-bound}.

\paragraph{Paper organization.}
Section~\ref{sec:related} positions the work relative to personalized bandits, prompt evolution, model routing, and generalized-ridge meta-learning. Section~\ref{sec:method} presents the inherited notation, the base decomposition, and the hyper-ridge router. Section~\ref{sec:theory} states the effective-dimension theory, including Assumption~\ref{assump:shared-structure} and Proposition~\ref{prop:deff-bound}. Section~\ref{sec:experiments} reports the base-method results and specifies the planned hyper-ridge experiments. Section~\ref{sec:conclusion} discusses limitations, chiefly the open $\mathrm{Err}_T$ analysis and the absence of new-method empirical results. Proofs and proof-status statements for the hyper-ridge results appear in Appendix~\ref{app:hyper-proofs}.
...
A user $U \sim \Pi$ is drawn from a population.

The system operates over $T$ rounds starting from a common initial archive $A_1$.

Let $\mathcal{F}_{t-1} = \sigma(U, A_1, a_1, r_1, \ldots, a_{t-1}, r_{t-1})$ be the natural filtration generated by the user draw and all past actions and rewards through round $t-1$.
...
The router selects $a_t(U) \in A_t(U)$ and observes reward $r_t = \mu(U, a_t(U)) + \xi_t$, where $\mu(u,a) \in [0,1]$ is the mean reward and $\xi_t$ is centered noise.

The current-archive oracle value is $m_t(U) = \max_{a \in A_t(U)} \mu(U,a)$.

We define the four terms of \eqref{eq:method-thesis}.

The \emph{default reward} is $V_{\mathrm{static}}(A) \,=\, \max_{a \in A} \E_{U}[\mu(U,a)]$, the reward of the best single candidate served to everyone.

The \emph{oracle routing value} is $V_{\mathrm{route}}(A) \,=\, \E_{U}[\max_{a \in A} \mu(U,a)]$, the reward attainable if each user were matched to their best candidate.

Their difference is the \emph{routing gain} $\Delta_{\mathrm{route}}(A) \,=\, V_{\mathrm{route}}(A) - V_{\mathrm{static}}(A) \,\geq\, 0$.

These motivate \emph{why} personalization matters but not \emph{when} it pays off; the routing gain $\Delta_{\mathrm{route}}$ is exactly that quantity.

The \emph{evolution gain} at round $t$ is $G_t(U) \,=\, m_t(U) - m_1(U)$, measuring how much the archive has improved since initialization.

The \emph{exploration cost} is $L_t(U) \,=\, m_t(U) - \mu(U, a_t(U))$, the gap between the current-archive best and what the router actually picked.
...
The remainder of the paper turns to empirical evidence. The next section
describes the evaluation environments and reports results for the base
PersonalEvo router; the corresponding experiments for Hyper-Ridge
PersonalEvo, which require estimating $\widehat{\Omega}_t$ from archive
data, are specified there as a planned protocol and have not yet been run.
...
replaces the base bound $\sqrt{\Kmax\, d\, T \log T}$. Both of these are
stated here as propositions rather than theorems, since a complete proof
controlling the covariance-estimation term
$\Err_T(\widehat{\Omega}_t, \Omega)$ has not yet been written. The improvement
is meaningful under an explicit regime condition, which we state as an
...
\zz{just move this below subsection into the same subsection of Theory: improved exploration cost }
...
Throughout this section the base assumptions of Section~\ref{sec:background} (Assumptions~\ref{ass:bounded}--\ref{ass:bounded-feat}) remain in force, and we write $x(u,a)$ for the feature map denoted $\psi(u,a)$ there; the only new hypothesis is the shared archive geometry stated below.
...
The four-term decomposition remains unchanged:
...
\midrule
General       & .570 & .280 & .051 & .036 & $\mathbf{+.244}$ \\
Workspace     & .577 & .124 & .041 & .043 & +.081 \\
Support       & .579 & .215 & .061 & .039 & +.176 \\
GEPA archive  & .654 & .048 & .023 & .025 & +.022 \\
Hermes agent  & .703 & .092 & .036 & .028 & +.065 \\
BESPOKE       & .653 & .036 & .001 & .010 & +.027 \\
PAHF shop.    & .577 & .209 & .002 & .054 & +.156 \\
...
\begin{enumerate}
\item \textbf{Routing gain dominates.} $\Delta_{\mathrm{route}}$ accounts for 85--95\% of the improvement over the static baseline in every row. The ordering matches user heterogeneity: General (5 disjoint peaks) $>$ PrefEval $>$ PAHF $>$ Support $>$ Workspace $>$ BESPOKE (mild heterogeneity), consistent with Lemma~\ref{lem:hetero}.
\item \textbf{Dominance is always positive.} On all nine benchmarks, including four with real user preference data, the condition $\Delta_{\mathrm{route}}+\bar G_T>\bar L_T$ from Corollary~\ref{cor:dominance} holds. The margin ranges from $+0.022$ (GEPA, mild heterogeneity) to $+0.244$ (General, strong cluster structure).
...
\subsection{Planned evaluation of Hyper-Ridge PersonalEvo}
\label{sec:exp-hyper-ridge-plan}

All numbers reported in this section---the decomposition estimates in Table~\ref{tab:main}, the ablations in Figure~\ref{fig:ablation}, the per-round traces in Figure~\ref{fig:perround-matrix}, and the baseline comparison in Table~\ref{tab:baselines}---are results for the \emph{base} PersonalEvo method, reproduced from the base result artifacts. No result artifacts exist for the hyper-ridge router, and we therefore make no empirical claim that hyper-ridge PersonalEvo improves on, matches, or differs from the base method; the evaluation below is specified at the implementation-ready level only.

\paragraph{Planned protocol.} The planned comparison runs hyper-ridge PersonalEvo and base PersonalEvo under identical conditions on the same nine benchmarks used throughout this section (General, Workspace, Support, GEPA archive, Hermes agent, BESPOKE, PAHF shopping, PAHF embodied, PrefEval), with 5 seeds per cell and the reward model of \eqref{eq:exp-mu}. The pre-registered metrics are: (i)~mean reward over 5 seeds per benchmark, in the format of Table~\ref{tab:baselines}; (ii)~the four decomposition terms of \eqref{eq:method-thesis}, with $\widehat L_T$ the primary endpoint since the hyper-ridge modification targets exploration cost; (iii)~the fitted log-log regret exponent, to test whether $\widehat L_T$ scales with an estimated $d_{\mathrm{eff}}(\widehat\Omega)$ rather than $d$; and (iv)~a plug-in estimate of $d_{\mathrm{eff}}(\widehat\Omega)$ per benchmark, to check the shared-structure regime assumption. The run would emit artifacts \texttt{hyper\_ridge\_per\_seed.csv}, \texttt{hyper\_ridge\_decomposition\_summary.csv}, \texttt{hyper\_ridge\_vs\_base\_9bench.json}, and figures \texttt{fig\_hyper\_ridge\_regret\_loglog.png} and \texttt{fig\_hyper\_ridge\_deff\_per\_benchmark.png}; none of these artifacts currently exists.

\paragraph{Implementation status.} The current codebase implements the base method. Converting it to hyper-ridge PersonalEvo is a localized change to two components of Algorithm~\ref{alg:router}: the per-user ridge estimator (replacing the identity-regularized solve with an $\widehat\Omega$-weighted regularizer) and the router's shared-statistics accumulator (adding the cross-user covariance estimate $\widehat\Omega$ updated from pooled residuals). The environment simulators, decomposition estimators, and evaluation harness require no modification, so the planned runs reuse the exact pipeline that produced the base-method artifacts.
...
\mathrm{Err}_T(\widehat\Omega, \Omega).
\end{equation}
When $d_{\mathrm{eff}}(\Omega) \ll d$, the dominance condition \eqref{eq:conc-dom} is satisfied in strictly more regimes, so archive evolution contributes twice: newly evolved candidates raise $\bar G_T$, and the data they generate sharpen $\widehat\Omega_t$ and lower $\bar L_T$. This is the paper's one thesis, and every component of the revision --- the generalized-ridge estimator, the effective-dimension bound, and the planned evaluation grid --- is an instance of it.

Two provenance facts bound what this paper claims. First, all reported empirical results --- the nine-benchmark decomposition estimates, the ablation hierarchy, the per-round convergence traces, and the baseline comparison in Section~\ref{sec:experiments} --- belong to the \emph{base} LinUCB router. They validate the inherited decomposition \eqref{eq:conc-decomp} and dominance condition \eqref{eq:conc-dom}, with margins
\begin{equation}
\label{eq:conc-margins}
\Delta_{\mathrm{route}}(A_1) + \bar G_T - \bar L_T \;\in\; [+0.022,\, +0.244]
\end{equation}
across all nine benchmarks, and they confirm that the routing gain accounts for $85$--$95\%$ of the improvement over the best static candidate. They say nothing about the hyper-ridge router. Second, the effective-dimension guarantee \eqref{eq:conc-hyper-rate} is a theorem for the oracle-covariance router but only a conditional statement for the estimated-covariance router: the term $\mathrm{Err}_T(\widehat\Omega,\Omega)$ is not yet controlled by a self-contained finite-sample analysis, and we have stated the estimated-covariance result as a proposition with that gap named rather than as an unconditional theorem. Closing this gap --- a concentration bound for the pooled-residual estimator $\widehat\Omega_t$ under the bandit's adaptive sampling --- is the primary theoretical open problem left by this revision.

The corresponding empirical open problem is the planned comparison of Section~\ref{sec:exp-hyper-ridge-plan}: base LinUCB versus oracle-covariance, estimated-covariance, and diagonal-ridge routers on the same nine benchmarks, with the exploration cost $\widehat L_T$ as the primary endpoint, the fitted log-log regret exponent as the scaling diagnostic, and a per-benchmark plug-in estimate of $d_{\mathrm{eff}}(\widehat\Omega)$ to check the shared-structure assumption. The heterogeneity and low-rank geometry sweeps in that plan are designed to locate both sides of the boundary: regimes where
\begin{equation}
\label{eq:conc-regime}
d_{\mathrm{eff}}(\Omega) \ll d
\end{equation}
and the hyper-ridge router should dominate, and near-isotropic regimes where $d_{\mathrm{eff}}(\Omega)\approx d$ and the extra covariance estimation can only cost. The implementation is a localized change to two components of Algorithm~\ref{alg:router} --- the per-candidate ridge solve and a shared covariance accumulator --- so the planned runs reuse the exact harness that produced the base-method artifacts.
...
Conjecture~\ref{conj:err-control} is the only unresolved step in turning the estimated-covariance statement into a full theorem.  The current repository implements the base per-candidate ridge router, not the hyper-covariance estimator; therefore this conjecture is stated as a proof obligation and as the target of the planned oracle-vs-estimated covariance experiments in Section~\ref{sec:experiments}, not as an empirical claim.
...
Empirical validation of hyper-ridge gains & oracle, estimated, diagonal, and base routers & planned in Section~\ref{sec:experiments} \\
\bottomrule
\end{tabular}
